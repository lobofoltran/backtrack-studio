...
function diffSecsMinHourYears($data)
{
	$d = new DateTime($data);
	$diff = $d->diff(new DateTime(date('Y-m-d H:i:s')));
	if ($diff->y > 1) {
		$data = "$diff->y anos atrás";
	} else if ($diff->y == 1) {
		$data = "$diff->y ano atrás";
	} else {
		if ($diff->m > 1) {
			$data = "$diff->m meses atrás";
		} else if ($diff->m == 1) {
			$data = "$diff->m mês atrás";
		} else {
			if ($diff->d > 1) {
				$data = "$diff->d dias atrás";
			} else if ($diff->d == 1) {
				$data = "$diff->d dia atrás";
			} else {
				if ($diff->h > 1) {
					$data = "$diff->h horas atrás";
				} else if ($diff->h == 1) {
					$data = "$diff->h hora atrás";
				} else {
					if ($diff->i > 1) {
						$data = "$diff->i minutos atrás";
					} else if ($diff->i == 1) {
						$data = "$diff->i minuto atrás";
					} else {
						if ($diff->s > 1) {
							$data = "$diff->s segundos atrás";
						} else if ($diff->s == 1) {
							$data = "$diff->s segundos atrás";
						} else {
							$data = "$diff->s segundos atrás";
